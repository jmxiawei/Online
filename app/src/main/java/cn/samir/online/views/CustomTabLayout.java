...
import cn.samir.online.util.ReflectUtils;
...
public class CustomTabLayout extends HorizontalScrollView {
    ...
    float mTabTextSize = 24;
    float mTabTextMultiLineSize = 20;

    final int mTabBackgroundResId;

    int mTabMaxWidth = Integer.MAX_VALUE >> 2;
    private final int mRequestedTabMinWidth = 40;
    private final int mRequestedTabMaxWidth = Integer.MAX_VALUE >> 4;
    private final int mScrollableTabMinWidth = 60;

    private int mContentInsetStart;

    int mTabGravity = GRAVITY_FILL;
    ...
    private CustomTabLayout.CustomTabLayoutOnPageChangeListener mPageChangeListener;
    ...
    public CustomTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabStrip.setSelectedIndicatorHeight(DensityUtil.dip2px(context, 4));
        mTabStrip.setSelectedIndicatorColor(Color.BLACK);
        // mTabStrip.setBackgroundResource(R.color.black);
        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = 0;
        mTabPaddingStart = 0;
        mTabPaddingTop = 0;
        mTabPaddingEnd = 0;
        mTabPaddingBottom = 0;
        mTabBackgroundResId = R.color.color_white;
        /**
         *  app:tabGravity="fill"
        ...
        mTabTextSize = DensityUtil.dip2px(context, 12);
        mTabTextColors = createColorStateList(getResources().getColor(R.color.text_color_gray), getResources().getColor(R.color.text_color_black));
    ...
    private void addTabFromItemView(@NonNull TabItem item) {
        ...
        CharSequence mText = (CharSequence) ReflectUtils.getField(item, "mText");
        if (mText != null) {
            tab.setText(mText);
        }

        Drawable mIcon = (Drawable) ReflectUtils.getField(item, "mIcon");
        if (mIcon != null) {
            tab.setIcon(mIcon);
        }

        int mCustomLayout = (int) ReflectUtils.getField(item, "mIcon");

        if (mCustomLayout != 0) {
            tab.setCustomView(mCustomLayout);
        }
    ...
    private void setupWithViewPager(@Nullable final ViewPager viewPager, boolean autoRefresh,
        ...
        if (viewPager != null) {
            ...
            if (mPageChangeListener == null) {
                mPageChangeListener = new CustomTabLayout.CustomTabLayoutOnPageChangeListener(this);
            }
            mPageChangeListener.reset();
            viewPager.addOnPageChangeListener(mPageChangeListener);

            // Now we'll add a tab selected listener to set ViewPager's current item
            mCurrentVpSelectedListener = new CustomTabLayout.ViewPagerOnTabSelectedListener(viewPager);
            ...
            if (mAdapterChangeListener == null) {
                mAdapterChangeListener = new CustomTabLayout.AdapterChangeListener();
            }
    ...
    void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver) {
        ...
        if (addObserver && adapter != null) {
            ...
            if (mPagerAdapterObserver == null) {
                mPagerAdapterObserver = new CustomTabLayout.PagerAdapterObserver();
            }
    ...
    private CustomTabLayout.TabView createTabView(@NonNull final CustomTabLayout.Tab tab) {
        ...
        if (tabView == null) {
            tabView = new CustomTabLayout.TabView(getContext());
        }
        tabView.setTab(tab);
    ...
    class TabView extends LinearLayout implements OnLongClickListener {
        private CustomTabLayout.Tab mTab;
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            super.draw(canvas);
            //LogUtil.e("mIndicatorLeft=" + mIndicatorLeft + ",mIndicatorRight=" + mIndicatorRight);
            // Thick colored underline below the current selection
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                // TODO: 2017/5/10 0010 加上

                int left = (mIndicatorRight - mIndicatorLeft - mIndicatorWidth) / 2 + mIndicatorLeft;
    ...
    public static class CustomTabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public CustomTabLayoutOnPageChangeListener(CustomTabLayout tabLayout) {
            mCustomTabLayoutRef = new WeakReference<>(tabLayout);
        }
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final CustomTabLayout tabLayout = mCustomTabLayoutRef.get();
            if (tabLayout != null) {
