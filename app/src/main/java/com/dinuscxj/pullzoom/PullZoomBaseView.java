...
public abstract class PullZoomBaseView<T extends View> extends LinearLayout {
    public static final long ZOOM_BACK_DURATION = 300L;
    ...
    private ArrayList<OnPullZoomListener> mOnPullZoomListeners = new ArrayList<>();
    ...
    private boolean onPullStartActionMove(MotionEvent event) {
        ...
        notifyPullZoomOnPullZooming(scrollValue);
    ...
    private boolean onPullStartActionCancel() {
        ...
        if (isZooming) {
            ...
            notifyPullZoomOnPullZoomEnd(scrollValue);
    ...
    public void notifyPullZoomOnPullStart() {
        int size = this.mOnPullZoomListeners.size();
        for (int i = 0; i < size; i++) {
            OnPullZoomListener mOnPullZoomListener = this.mOnPullZoomListeners.get(i);
    ...
    public void notifyPullZoomOnPullZooming(float newScrollValue) {
        int size = this.mOnPullZoomListeners.size();
        for (int i = 0; i < size; i++) {
            OnPullZoomListener mOnPullZoomListener = this.mOnPullZoomListeners.get(i);
            if (mOnPullZoomListener != null) {
    ...
    public void notifyPullZoomOnPullZoomEnd(float newScrollValue) {
        int size = this.mOnPullZoomListeners.size();
        for (int i = 0; i < size; i++) {
            OnPullZoomListener mOnPullZoomListener = this.mOnPullZoomListeners.get(i);
            if (mOnPullZoomListener != null) {
                mOnPullZoomListener.onPullZoomEnd(newScrollValue);
            }
        }
    }


    public void addOnPullZoomListener(OnPullZoomListener mOnPullZoomListener) {

        if (!this.mOnPullZoomListeners.contains(mOnPullZoomListener)) {
            this.mOnPullZoomListeners.add(mOnPullZoomListener);
        }
    }
    ...
    public interface OnPullZoomListener {
        void onPullUp(float newScrollValue);
